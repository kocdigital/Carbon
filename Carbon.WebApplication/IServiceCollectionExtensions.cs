                ...
                .AddJwtBearer("Bearer", options =>
                {
                    ...
                    if (tvs != null && tvs.Enabled)
                    {
                        ...
                        baseEvents.OnTokenValidated = async ctx =>
                        {
                            ...
                            // If the JWT does not contain a TokenVersion claim (legacy token)
                            if (tokenVersionClaim == null)
                            {
                                // Skip validation during the migration period and consider the token valid
                                return;
                            }

                            // If the claim exists but cannot be parsed as a number (invalid data state)
